        ...
        {links.map((link) => (
          <li key={link.href}>
            <Link
              ...
              className="inline-block text-[15px] leading-snug text-white/90 transition-colors hover:text-white"
            >
              {link.label}
            </Link>
          </li>
  ...
  return (
    <footer className="relative z-20 overflow-hidden bg-black text-white">
      <div className="mx-auto max-w-container px-4 sm:px-6 lg:px-8">
        {/* Link columns */}
        <div className="grid grid-cols-2 gap-x-10 gap-y-10 py-16 sm:grid-cols-3 lg:grid-cols-5 lg:gap-x-12 lg:py-20">
        ...
        {/* Social + copyright */}
        <div className="flex flex-col-reverse items-start justify-between gap-6 border-t border-white/10 py-8 sm:flex-row sm:items-center">
      ...
      <div
        className="pointer-events-none relative mt-6 w-full overflow-hidden"
        style={{ height: "clamp(260px, 38vw, 480px)" }}
        aria-hidden
      >
        <p
          className="absolute left-1/2 w-[100vw] -translate-x-1/2 text-center font-heading font-bold leading-[0.76] tracking-[-0.05em] text-transparent"
          style={{
            bottom: "-6%",
            fontSize: "clamp(16rem, 34vw, 44rem)",
