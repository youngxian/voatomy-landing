...
const PHANTOM_INTEGRATIONS = INTEGRATION_CATALOG.filter((i) =>
  (i.products as readonly string[]).includes("phantom"),
);

const CODE_INTEGRATIONS       = PHANTOM_INTEGRATIONS.filter((i) => i.category === "Code");
const MONITORING_INTEGRATIONS = PHANTOM_INTEGRATIONS.filter((i) => i.category === "Monitoring");
...
function IntegrationButton({
  ...
  integration: (typeof PHANTOM_INTEGRATIONS)[number];
  connected: boolean;
  onConnect: () => void;
  onDisconnect: () => void;
  loading: boolean;
    ...
    <button
      ...
      className={cn(
        ...
        connected
          ...
          : "border-[#121312]/10 bg-white hover:border-[#6366F1]/30 hover:bg-[#6366F1]/5",
        loading && "opacity-50",
      )}
    >
      ...
      {loading ? (
        <svg className="animate-spin h-4 w-4 text-[#6366F1]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
...
export function PhantomConnectStep() {
  ...
  const handleContinue = async () => {
    ...
    try { await saveStep("phantom-connect", { connected_integrations: connected.map((c) => c.key) }); } catch { /* continue */ }
  ...
  return (
    <div>
      ...
      <div className="space-y-5">
        ...
        {MONITORING_INTEGRATIONS.length > 0 && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
            ...
            <p className="mb-2 text-xs text-fynk-muted">Connect monitoring to correlate debt hotspots with runtime failures.</p>
            <div className="space-y-2">
              {MONITORING_INTEGRATIONS.map((i) => (
                <IntegrationButton
                  key={i.key}
                  integration={i}
                  connected={isConnected(i.key as IntegrationKey)}
