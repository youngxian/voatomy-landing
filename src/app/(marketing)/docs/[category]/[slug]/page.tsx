...
export function generateMetadata({ params }: { params: { category: string; slug: string } }): Metadata {
  const doc = DOCS_DATA[params.category]?.[params.slug];
  if (!doc) {
    return { title: "Doc Not Found — Voatomy Docs" };
  }
  const url = `${SITE_CONFIG.url}/docs/${params.category}/${params.slug}`;
  return {
...
export { DOCS_DATA, SIDEBAR_NAV };

export default function Page({ params }: { params: { category: string; slug: string } }) {
  return <PageContent category={params.category} slug={params.slug} />;
}
